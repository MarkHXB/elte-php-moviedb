...
if(count($_POST) !== 0){
    if(!postKeyIsValid("username")){
        ...
        $errors["username"] = "Már létezik ilyen felhasználónév az adatbázisban!";
    }else{
        $data["username"] = $_POST["username"];
    }

    if(!postKeyIsValid("email")){
      $errors["email"] = "Nem adtál meg email címet!";
    }else if(!checkemail($_POST["email"])){
      $errors["email"] = "Nem helyesen adtad meg az email címet!";
    } else{
      $data["email"] = $_POST["email"];
    }

    if(!postKeyIsValid("password")){
        $errors["password"] = "Nem adtál meg jelszót!";
    }else if(!postKeyIsValid("password2")){
        $errors["password2"] = "Nem adtál meg jelszót!";
              ...
              <div class="col-md-10 col-lg-6 col-xl-5 order-2 order-lg-1">
                ...
                <form method="post" class="mx-1 mx-md-4" novalidate>
                  ...
                  <div class="d-flex flex-row align-items-center mb-4">
                    ...
                    <div class="form-outline flex-fill mb-0">
                      <input type="password" name="password2" id="form-password2" class="form-control" 
                      value="<?= $_POST['password2'] ?? '' ?>"/>
                      <label class="form-label" for="form-password2">Jelszó újra</label>
                      <?= error( $errors["password2"] ?? "") ?>
                    </div>
                  </div>

                  <div class="d-flex justify-content-center mx-4 mb-3 mb-lg-4">
                    <button type="submit" class="btn btn-primary btn-lg">Regisztrál</button>
                  </div>
